...
/// </summary>
public class TableroManager : SerializedMonoBehaviour {
    /// <summary>
    ...
    /// <summary>
    /// Elemento al que se llamará para configurar los escenarios una vez se creen los pasillos.
    /// </summary>
    private EscenariosManager escenariosManager;
    ...
    private void generateCasillas() {
        ...
        escenariosManager.loadEscenarios();

        int doorsCount = puertasManager.getDoorsCount();
        int escenariosCount = escenariosManager.getEscenariosCount();

        for(int i = 0; i < escenariosCount; i++) {
            //GENERAR ESCENARIOS
        }
        ...
        for (int i = 0; i < columns; i++) {
            for (int j = 0; j < rows; j++) {
                if(casillasPasillo[i, j]) {
                    // Inicialización base de casilla
                    GameObject posCasilla = makeBaseCasilla(i, j);
                    int indexDoor = -1;

                    if(doorsCount > 0) {
                        indexDoor = puertasManager.searchDoor(i, j);
                    }

                    if(indexDoor != -1) { // Es casilla puerta
                        //Creación del stat de la casilla
                        makeStatCasilla(i, j, Tipo_Casilla.PUERTA, posCasilla, indexDoor);

                        // Adición de casilla puerta
                        gridArray[i, j] = posCasilla;
                        doorsCount--;

                        //TODO: Falta añadir la casilla enlace con escenario
                    } else { // Es casilla pasillo
                        //Creación del stat de la casilla
                        makeStatCasilla(i, j, Tipo_Casilla.PASILLO, posCasilla, -1);

                        // Adición de casilla pasillo
                        gridArray[i, j] = posCasilla;
                    }
                }
            }
    ...
    /// <summary>
    /// Método que genera la base común de las casillas.
    /// </summary>
    /// <param name="i">Valor X del objeto en el espacio.</param>
    /// <param name="j">Valor Y del objeto en el espacio.</param>
    /// <returns>Casilla generada en el espacio.</returns>
    private GameObject makeBaseCasilla(int i, int j) {
    ...
    private void makeStatCasilla(int i, int j, Tipo_Casilla tipo, GameObject casilla, int indexDoor) {
        if(tipo == Tipo_Casilla.PASILLO) {
            casilla.AddComponent<CasillaStat>();
            CasillaStat casillaStat = casilla.GetComponent<CasillaStat>();
            casillaStat.initCasilla(i, j, tipo);
        } else if(tipo == Tipo_Casilla.PUERTA) {
            casilla.AddComponent<PuertaStat>();
            PuertaStat doorStat = casilla.GetComponent<PuertaStat>();

            Coordenada doorPos = puertasManager.getCoordenadaPuerta(indexDoor);
            Orientacion doorOri = puertasManager.getOrientacionPuerta(indexDoor);

            doorStat.initCasilla(doorPos.getX(), doorPos.getY(), tipo, doorOri);
        }
    }
    ...
    /// <summary>
    /// Setter del mánager de las puertas.
    /// </summary>
    /// <param name="pManager">Mánager de las puertas.</param>
    public void setPuertasManager(PuertasManager pManager) {
        puertasManager = pManager;
    }

    /// <summary>
    /// Setter del mánager de los escenarios.
    /// </summary>
    /// <param name="eManager">Mánager de los escenarios.</param>
    public void setEscenariosManager(EscenariosManager eManager) {
        escenariosManager = eManager;
    }
}
